...
int ansS, ansT; // Declare ansS and ansT as global variables
...
int calculateE(long long phi) {
   int e = 2;  // Start with a small e value
   while (e < phi) {
       if (findGCD(e, phi) == 1)
           return e;
       e++;
...
int main() {
   long long p, q, msg;
   cout << "Please enter 2 prime numbers and Message to Encrypt" << endl;
   cin >> p >> q >> msg;

   cout << "2 random prime numbers selected are " << p << " " << q << endl;

   long long n = p * q;
   cout << "Product of two prime numbers n is " << n << endl;
   ...
   if (e == 0) {
       cout << "No suitable e found. Please choose different prime numbers." << endl;
       return 1;
   }
   cout << "Final e value is " << e << endl;
